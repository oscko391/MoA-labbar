    ...
    for (int j = 0; j < mVoxels.GetDimY(); j++) {
      for (int k = 0; k < mVoxels.GetDimZ(); k++) {
        ...
        // TODO: Add code here
		  ...
		  if (IsFluid(i, j, k))
		  {
			  Vector3<float> F = mExternalForces->GetValue(x, y, z);
			  Vector3<float> V1 = mVelocityField.GetValue(i, j, k);
			  Vector3<float> V2 = V1 + dt*F;
  ...
  for (int i = 0; i < mVoxels.GetDimX(); i++) {
    for (int j = 0; j < mVoxels.GetDimY(); j++) {
      for (int k = 0; k < mVoxels.GetDimZ(); k++) {
        ...
        // TODO: Add code here
		  vec = mVelocityField.GetValue(i, j, k);

		  if (IsFluid(i,j,k))
		  {
			  //X
			  if (IsSolid(i - 1, j, k) && vec[0] < 0)
				  vec[0] = 0;
			  if (IsSolid(i + 1, j, k) && vec[0] > 0)
				  vec[0] = 0;
			  //Y
			  if (IsSolid(i, j-1, k) && vec[1] < 0)
				  vec[1] = 0;
			  if (IsSolid(i, j+1, k) && vec[1] > 0)
				  vec[1] = 0;
			  //Z
			  if (IsSolid(i, j, k-1) && vec[2] < 0)
				  vec[2] = 0;
			  if (IsSolid(i, j, k+1) && vec[2] > 0)
				  vec[2] = 0;

			  mVelocityField.SetValue(i, j, k, vec);
		  }
    ...
    for (int j = 0; j < mVoxels.GetDimY(); j++) {
      for (int k = 0; k < mVoxels.GetDimZ(); k++) {
        ...
        if (IsFluid(i,j,k)) {
		  ...
		  /*float velocityDiff = ((mVelocityField.GetValue(i+1, j, k)[0] + mVelocityField.GetValue(i, j+1, k)[1] + mVelocityField.GetValue(i, j, k+1)[2])
			  - (mVelocityField.GetValue(i - 1, j, k)[0] + mVelocityField.GetValue(i, j - 1, k)[1] + mVelocityField.GetValue(i, j, k - 1)[2]))/(2.0*mDx);*/

		  float u = mVelocityField.GetValue(i + 1, j, k)[0] - mVelocityField.GetValue(i - 1, j, k)[0];
		  float v = mVelocityField.GetValue(i , j + 1, k)[1] - mVelocityField.GetValue(i , j - 1, k)[1];
		  float w = mVelocityField.GetValue(i , j, k + 1)[2] - mVelocityField.GetValue(i , j, k - 1)[2];
		  ...
		  //eq: 22
		  int arr[7] = { !IsSolid(i + 1,j,k), !IsSolid(i - 1,j,k),!IsSolid(i,j + 1,k), 0, !IsSolid(i,j - 1,k), !IsSolid(i,j ,k + 1), !IsSolid(i,j ,k - 1) };
			
		  A(ind, ind_ip) = arr[0] / dx2;
		  A(ind, ind_im) = arr[1] / dx2;
		  A(ind, ind_jp) = arr[2] / dx2;
		  
		  A(ind, ind_jm) = arr[4] / dx2;
		  A(ind, ind_kp) = arr[5] / dx2;
		  A(ind, ind_km) = arr[6] / dx2;
    ...
    for (int j = 0; j < mVoxels.GetDimY(); j++) {
      for (int k = 0; k < mVoxels.GetDimZ(); k++) {
        ...
        if (IsFluid(i,j,k)) {
          ...
          // TODO: Add code here
		  Vector3<float> xGrad;
		  xGrad[0] = (x[ind_ip] - x[ind_im]) /(2*mDx);
		  xGrad[1] = (x[ind_jp] - x[ind_jm]) / (2 * mDx);
		  xGrad[2] = (x[ind_kp] - x[ind_km]) / (2 * mDx);
		  Vector3<float> diff = mVelocityField.GetValue(i,j,k) - xGrad;
		  mVelocityField.SetValue(i, j, k, diff);
        }
      }
    }
  }
