...
public :
  ...
  virtual float ComputeTimestep()
  {
	...
	// delta t < min(deltaX/|Vx|, deltaY/|Vy|, deltaZ/|Vz|)
	  float result = mLS->GetDx() / abs(mF);
	  return result;
  }
  ...
  virtual float Evaluate(unsigned int i, unsigned int j, unsigned int k)
  {
    // Compute the rate of change (dphi/dt)
	  float ddx, ddy, ddz;
	  Godunov(i, j, k, mF , ddx, ddy, ddz);

	  // delta phi/dt = -F|grad(phi)|
	  return -mF * (std::sqrt(ddx + ddy + ddz));
  }
