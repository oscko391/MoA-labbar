...
Vector3<float> LoopSubdivisionMesh::VertexRule(unsigned int vertexIndex)
{
  ...
  std::vector<unsigned int> innerRing = FindNeighborVertices(vertexIndex);

  int n = innerRing.size();

  //float w = (5 / 8.0) - (pow(3 + 2 * cos(2 * M_PI / n), 2) / 64.0);

  float b = Beta(innerRing.size());

  vtx *= (1 - b*n);

  for (int i = 0; i < innerRing.size(); i++)
  {
	  vtx += v(innerRing[i]).pos * b;
  }

  std::cout << "vertex rule" << std::endl;
...
Vector3<float> LoopSubdivisionMesh::EdgeRule(unsigned int edgeIndex)
{
  ...
  return ((3/8.0) * (v0 + v1) + (1/8.0)*(v2 + v3));
}
